...
const CategorySidebar: React.FC<CategorySidebarProps> = ({ 
  ...
  const handleMainCategoryClick = (slug: string) => {
    setExpandedMainCategory(prev => (prev === slug ? null : slug)); // Toggle expansion
    onCategorySelect(slug, null); // Select main category, clear sub-category
  };

  const handleSubCategoryClick = (mainSlug: string, subSlug: string) => {
    setExpandedMainCategory(mainSlug); // Ensure parent is expanded
  ...
  return (
    <div className="bg-bgBase rounded-lg shadow-md border border-borderDefault h-full flex flex-col">
      ...
      <nav className="flex-grow p-3 space-y-1 overflow-y-auto">
        ...
        {PRODUCT_CATEGORIES_HIERARCHY.filter(cat => cat.name !== "PC Xây Dựng").map((mainCat) => (
         ...
         <div className="pt-2 mt-2 border-t border-borderDefault">
            <Link
                to="/shop?promo=true" // Example promo link
