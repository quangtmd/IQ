...
const FeaturedProjects: React.FC = () => {
  const featuredProjects = MOCK_PROJECTS.slice(0, 3); // Show first 3 projects
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {featuredProjects.map((project: Project) => (
            <Card key={project.id} className="flex flex-col overflow-hidden h-full border border-borderDefault hover:shadow-xl transition-shadow duration-300">
              <Link to={`/projects`} className="block aspect-video overflow-hidden"> {/* Or to individual project page: /project/${project.id} */}
                <img 
              ...
              <div className="p-6 flex flex-col flex-grow">
                ...
                <Link to={`/projects`} className="block"> {/* Or to individual project page */}
