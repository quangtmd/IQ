...
export const AuthProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    ...
    if (storedUsers.length === 0 || !storedUsers.find(u => u.email === ADMIN_EMAIL && u.role === 'admin')) {
      ...
      storedUsers = storedUsers.filter(u => u.email !== ADMIN_EMAIL);
      storedUsers.unshift(adminUser); 
      saveRegisteredUsers(storedUsers);
      if (storedUsers.length === 1) { 
          addAdminNotification(`Tài khoản quản trị (${ADMIN_EMAIL}) đã được tạo tự động.`, 'success');
      }
    }
  ...
  const updateUser = async (userId: string, updates: Partial<User>): Promise<boolean> => {
    ...
    if (usersToUpdate[userIndex].email === ADMIN_EMAIL && (updates.role && updates.role !== 'admin' || updates.email && updates.email !== ADMIN_EMAIL)) {
        addAdminNotification("Không thể thay đổi vai trò hoặc email của quản trị viên chính.", "error");
        return false;
    }
    if (updates.email === ADMIN_EMAIL && usersToUpdate[userIndex].email !== ADMIN_EMAIL) {
        addAdminNotification(`Không thể đặt email thành ${ADMIN_EMAIL} cho người dùng này.`, "error");
        return false;
    }
  ...
  const deleteUser = async (userId: string): Promise<boolean> => {
     ...
     if (userToDelete.email === ADMIN_EMAIL && userToDelete.role === 'admin') {
  ...
  const hasPermission = (requiredPermissions: Array<AdminPermission>): boolean => {
    ...
    if (currentUser.role === 'staff' && currentUser.staffRole) {
        const staffPermissionsMap: Record<StaffRole, AdminPermission[]> = {
            'Sales Manager': ['viewSales', 'viewOrders', 'manageOrders', 'manageDiscounts', 'viewNotifications', 'viewProducts', 'viewCustomers'],
            'Content Editor': ['viewContent', 'viewArticles', 'manageArticles', 'manageFaqs', 'viewNotifications'],
            'Technical Lead': ['viewContent', 'viewProducts', 'manageProducts', 'viewNotifications', 'viewSupport'], 
            'Support Specialist': ['viewSales', 'viewOrders', 'viewNotifications', 'viewSupport', 'viewCustomers', 'manageFaqs'], 
            'Full Access Staff': [
        ...
        const userStaffPermissions = staffPermissionsMap[currentUser.staffRole] || [];
