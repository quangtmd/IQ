...
import { CheckoutFormData, Order } from '../types'; // Added Order
import { MOCK_ORDERS } from '../data/mockData'; // To simulate adding to orders

const CheckoutPage: React.FC = () => {
  const { cart, getTotalPrice, clearCart } = useCart();
  const { currentUser, isAuthenticated, addAdminNotification } = useAuth(); // Added addAdminNotification
  ...
  useEffect(() => {
    if (isAuthenticated && currentUser) {
      setFormData(prev => ({ ...prev, email: currentUser.email, fullName: currentUser.username })); // Pre-fill username as fullName
    }
  ...
  const handleSubmitOrder = (e: React.FormEvent) => {
    ...
    // Simulate adding to a mock order list (in a real app, this would be a backend call)
      ...
      <div className="container mx-auto px-4 py-12 text-center">
        <div className="max-w-lg mx-auto bg-bgBase p-8 rounded-lg shadow-xl border border-borderDefault">
          ...
          <p className="text-textMuted mb-6">
            Cảm ơn {formData.fullName} đã đặt hàng tại {COMPANY_NAME}. 
