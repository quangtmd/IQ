            ...
            <div className="bg-bgBase p-6 rounded-lg shadow-md border border-borderDefault">
              ...
              <div className="aspect-w-16 aspect-h-9 rounded-lg overflow-hidden shadow-sm border border-borderDefault">
                <iframe 
                  ...
                  title={`Google Maps Location - ${settings.companyName}`}
                ></iframe>
              </div>
            </div>
          )}
        ...
        <div className="bg-bgBase p-6 md:p-8 rounded-lg shadow-xl border border-borderDefault">
          ...
          {isSubmitted ? (
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <div><label htmlFor="name" className="block text-sm font-medium text-textMuted mb-1">Họ và tên *</label><input type="text" name="name" id="name" value={formData.name} onChange={handleChange} required className="w-full p-3 input-style" /></div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                <div><label htmlFor="email" className="block text-sm font-medium text-textMuted mb-1">Email *</label><input type="email" name="email" id="email" value={formData.email} onChange={handleChange} required className="w-full p-3 input-style" /></div>
                <div><label htmlFor="phone" className="block text-sm font-medium text-textMuted mb-1">Số điện thoại *</label><input type="tel" name="phone" id="phone" value={formData.phone} onChange={handleChange} required className="w-full p-3 input-style" /></div>
              </div>
              <div><label htmlFor="subject" className="block text-sm font-medium text-textMuted mb-1">Chủ đề *</label><input type="text" name="subject" id="subject" value={formData.subject} onChange={handleChange} required className="w-full p-3 input-style" /></div>
              <div><label htmlFor="message" className="block text-sm font-medium text-textMuted mb-1">Nội dung tin nhắn *</label><textarea name="message" id="message" rows={6} value={formData.message} onChange={handleChange} required className="w-full p-3 input-style"></textarea></div>
