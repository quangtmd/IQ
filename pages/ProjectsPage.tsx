...
import { Link } from 'react-router-dom'; // If projects have detail pages
          ...
          {MOCK_PROJECTS.map(project => (
            <Card key={project.id} className="flex flex-col overflow-hidden h-full border border-borderDefault hover:shadow-xl transition-shadow duration-300">
              ...
              <div className="p-6 flex flex-col flex-grow">
                ...
                {/* Optional: Link to a detailed project page if available */}
                {/* <div className="mt-auto">
                  <Link to={`/project/${project.id}`} className="text-primary hover:underline text-sm font-medium">
                    Xem chi tiết <i className="fas fa-arrow-right text-xs ml-1"></i>
                  </Link>
                </div> */}
              </div>
            </Card>
