...
const PCBuilderPage: React.FC = () => {
        ...
        if (componentDetails) {
          ...
          components[selectorKey] = { name: compName, price: componentDetails.price };
           ...
           if (aiComp && typeof aiComp === 'object' && aiComp.name === compName) {
             components[selectorKey] = { name: compName, price: 0 }; // AI recommended, price might not be in MOCK_PC_COMPONENTS
           } else {
             components[selectorKey] = { name: compName, price: 0 }; // Fallback if not found
           }
        }
      }
    });
  ...
  const handleAddToCartAndNotify = () => {
    ...
    addToCart(customBuildItem as any); // Cast to any to bypass strict Product type if needed for CartItem
