...
import { USE_CASES, API_KEY_ERROR_MESSAGE, GENERIC_PC_BUILD_IMAGE_URL } from '../constants';
...
const selectorKeyToMockKeyMap: Record<BuilderSelectorKey, keyof typeof MOCK_PC_COMPONENTS | string> = { // Allow string for safety, though keys should match
  CPU: 'CPU',
...
const PCBuilderPage: React.FC = () => {
    ...
    (Object.keys(selectedComponents) as BuilderSelectorKey[]).forEach(selectorKey => {
      ...
      if (compName) {
        ...
        const componentList: PCComponent[] = (
          ...
          Array.isArray(MOCK_PC_COMPONENTS[mockKey]) // Ensure it's an array
  ...
  const handleAddToCartAndNotify = () => {
    ...
    const customBuildItem: CustomPCBuildCartItem = {
        ...
        price: totalCost,
        quantity: 1,
        imageUrl: GENERIC_PC_BUILD_IMAGE_URL,
        imageUrls: [GENERIC_PC_BUILD_IMAGE_URL],
        isCustomBuild: true,
  ...
  const renderRecommendation = (componentAiKey: keyof Omit<AIBuildResponse, 'error'>) => {
    ...
    // Ensure compCandidate is an object and has the expected properties
    if (compCandidate && typeof compCandidate === 'object' && 'name' in compCandidate && 'reasoning' in compCandidate) {
        const comp = compCandidate as AIRecommendedComponent; // Safe to cast now
         return (
  ...
  try {
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <Card className="lg:col-span-1 p-6 space-y-6 h-fit sticky top-24 border border-borderDefault">
            ...
            {!process.env.API_KEY && <p className="text-xs text-warning-text mt-1">API Key chưa được cấu hình. Tính năng AI sẽ không hoạt động.</p>}
          </Card>
    ...
    console.error("PCBuilderPage render error:", renderError);
    return (
      <div className="container mx-auto px-4 py-8 text-center">
        <h1 className="text-2xl font-semibold text-danger-text">Lỗi hiển thị trang PC Builder</h1>
